...
#img_folder = '/home/mengmi/Projects/Proj_context1/Datasets/MSCOCO/testColor_img/'
#bin_folder = '/home/mengmi/Projects/Proj_context1/Datasets/MSCOCO/testColor_binimg/'
txt_folder = '../Datalist/'  #replace with your own directory as above
img_folder = '../Datasets/MSCOCO/testColor_img/' #replace with your own directory as above
bin_folder = '../Datasets/MSCOCO/testColor_binimg/' #replace with your own directory as above
split = 'test'
...
checkpoint = 'models/checkpoint_2.pth.tar'  # download from github and put it here; model checkpoint
word_map_file = '../Datalist/ClassLabelList.txt'  # word map, ensure it's the same the data was encoded with and the model was trained with
...
def evaluate():
    ...
    # For each image
    for i, (imgs, binimgs, blurs, labels) in enumerate(test_loader):
        ...
        # forward everything
        scores, alphas, clickS = decoder(encoder, transform, imgs, blurs, binimgs, click_steps, batch_size, imgsz, ClickRadius)
        #print('scores')
        #print(scores.shape)
        #print('alphas')
        #print(alphas.shape)
        ...
        #save results
        sio.savemat('results/trial_' + str(i+1) + '.mat', {'predicted_seq':np.asarray(predicted_seq, dtype=np.int32), 'alphas': alphas.numpy(), 'clickS':clickS})
        print('Iter: [{0}/{1}]\t'
